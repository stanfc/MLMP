...
# No-Adaptation baseline on ACDC with 19 classes merged into 10 OVSS super-classes.
# CPU thread limits — avoid OpenCV/MKL spawning hundreds of threads
# when many experiments share 256 cores (res=11 spawn errors).
export OMP_NUM_THREADS=4
export MKL_NUM_THREADS=4
...
# Class names directly used as CLIP prompts (see utils/class_extensions/acdc_10class.txt).
# CPU thread limits — avoid OpenCV/MKL spawning hundreds of threads
# when many experiments share 256 cores (res=11 spawn errors).
export OMP_NUM_THREADS=4
export MKL_NUM_THREADS=4
