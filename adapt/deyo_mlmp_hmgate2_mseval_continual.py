...
class DeYOMLMPHMGate2MSEvalContinual:
    def __init__(self, ovss_type, ovss_backbone, lr, classes, steps=1,
                 ...
                 reweight_plpd=True,
                 top_block_exclude=6,
                 ema_m=0.99, tta_flip=1, ms_scales=(0.75, 1.25),
        ...
        self.ms_scales = tuple(float(x) for x in ms_scales)
        self.steps = steps
    ...
    @torch.no_grad()
    def _view(self, xin):
        lg = self._eval_forward(xin)
        if self.tta_flip:
            lg = 0.5 * (lg + self._eval_forward(torch.flip(xin, dims=[-1])).flip(dims=[-1]))
        return lg
    ...
    def evaluate(self, x):
        ...
        logits = self._view(x)                       # native scale = reference
        ref = logits.shape[-2:]
        cnt = 1
        for sc in self.ms_scales:                    # extra scales, resize logits back
            if abs(sc - 1.0) < 1e-6:
                continue
            xs = F.interpolate(x, scale_factor=sc, mode='bilinear', align_corners=False)
            lg = self._view(xs)
            if lg.shape[-2:] != ref:
                lg = F.interpolate(lg, size=ref, mode='bilinear', align_corners=False)
            logits = logits + lg
            cnt += 1
        logits = logits / cnt
        for n, p in self.named_ln_params:
