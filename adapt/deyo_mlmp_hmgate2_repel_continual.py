...
class DeYOMLMPHMGate2RepelContinual:
    def __init__(self, ovss_type, ovss_backbone, lr, classes, steps=1,
                 ...
                 reweight_plpd=True,
                 top_block_exclude=6,
                 lambda_repel=0.1, top_k_align=0.2, repel_k=3,
    ...
    def _align_loss(self, x):
        # single-layer forward -> per-pixel visual features + per-class text
        ...
        with torch.no_grad():
            ...
            conf, _ = probs.max(dim=1)                        # (B,w,h)
            k = max(1, int(round(conf.numel() * self.top_k_align)))
            thr = torch.topk(conf.flatten(), k, sorted=True).values[-1]
            mask = conf >= thr                                # confident pixels
            # hardest negatives: top (repel_k+1) logit classes, drop rank-0 (=pred)
            top_idx = avg_logits.topk(self.repel_k + 1, dim=1).indices  # (B,k+1,w,h)
            neg_idx = top_idx[:, 1:, :, :]                    # (B,k,w,h) confusable negatives
        B, _, w, h = avg_logits.shape
        D = image_features.shape[-1]
        vis = image_features[:, 1:, :].reshape(B, w, h, D)    # (B,w,h,D)
        neg_tgt = avg_text[neg_idx]                           # (B,k,w,h,D)
        neg_cos = (vis.unsqueeze(1) * neg_tgt).sum(dim=-1)    # (B,k,w,h)
        neg_cos = neg_cos.mean(dim=1)                         # (B,w,h) avg over k negatives
        # +cos in the loss -> minimizing PUSHES features away from confusable negatives
        return neg_cos[mask].mean()
            ...
            if mask_ent.sum() > 0:
                ...
                if final_mask.sum() > 0:
                    ...
                    if self.reweight_ent or self.reweight_plpd:
                        ...
                        loss = ent_kept.mean()
                    # --- REPULSION: push visual features AWAY from the top-k most
                    # confusable WRONG-class text anchors (uses inter-class text
                    # geometry -> non-redundant with entropy, unlike attraction). ---
                    if self.lambda_repel > 0.0:
                        loss = loss + self.lambda_repel * self._align_loss(x)
