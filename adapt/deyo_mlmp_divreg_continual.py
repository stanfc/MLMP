"""
DeYOMLMPDivRegContinual — DeYO-MLMP (deyo_mlmp_continual.py) with a
DIVERSITY REGULARIZER added directly to the adapt loss:

    L = L_DeYO  -  lambda_div * H_margin

where H_margin = -sum_c p_bar_c log p_bar_c is the entropy of the batch's
marginal class distribution p_bar (mean softmax over prompts, batch and
spatial positions). Maximizing H_margin (hence the minus sign) is the
SHOT-style information-maximization / equipartition term: it rewards the
model for *using all classes* and directly opposes the documented collapse
mode (1-2 dominant classes -> low H_margin -> mIoU crash).

Why H_margin (and not grad_norm, the single best degradation detector):
grad_norm is the strongest cross-dataset signal but Phase O-1 proved it is
a BAD loss target (high even when healthy; Hessian-vector penalty gradient
destabilises). H_margin is fully differentiable from the forward softmax,
and is the strongest *differentiable* signal on the collapse-regime
datasets (ACDC rho=+0.94, Cityscapes +0.98). On VOC20 (uniform-drift) it
is near-blind (rho~+0.14), so we expect gains mainly on ACDC/Cityscapes and
roughly neutral VOC20 at small lambda_div — this is the honest tradeoff.

lambda_div=0 is BIT-IDENTICAL to deyo_mlmp_continual (the term is skipped),
so this file doubles as its own control.

Base machinery (multi-prompt T=7, 18-layer mean-fusion adapt, UAML eval,
DeYO entropy+PLPD filter & reweight) is copied verbatim from
deyo_mlmp_continual.py; only the loss line and one ctor arg differ.
"""
import time
import copy
import math
...
class DeYOMLMPDivRegContinual:
    def __init__(self, ovss_type, ovss_backbone, lr, classes, steps=1,
        ...
        print(f"+++ DeYO-MLMP-DivReg (multi-prompt T={len(self.prompt_templates)}): "
              ...
              f"lambda_div={self.lambda_div} -> LN params: {len(params)}")
    ...
    def perform_adaptation(self, x):
        ...
        for _ in range(self.steps):
            ...
            # --- Diversity regularizer: maximize H_margin (anti-collapse) ---
            # prob (T,B,C,h,w) is differentiable (softmax of adapt logits, NOT
            # under no_grad). Marginal class distribution = mean softmax over
            # prompts, batch and space; its entropy is maximized by subtracting
            # lambda_div * H_margin from the loss. lambda_div=0 -> term skipped
            # -> bit-identical to deyo_mlmp_continual.
            h_margin_val = float('nan')
            if self.lambda_div != 0.0:
                marg = prob.mean(dim=(0, 1, 3, 4))            # (C,)
                marg = marg / marg.sum().clamp(min=1e-8)
                h_margin = -(marg * marg.clamp(min=1e-12).log()).sum()
                loss = loss - self.lambda_div * h_margin
                h_margin_val = float(h_margin.detach())
