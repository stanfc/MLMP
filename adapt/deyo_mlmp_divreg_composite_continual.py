"""
DeYOMLMPDivRegCompositeContinual — the COMBINED method: our diversity
regularizer (loss-side, deyo_mlmp_divreg_continual) PLUS the composite
restoration gate (optimizer-side, deyo_mlmp_composite_gate_continual).

    L = L_DeYO  -  lambda_div * H_margin        (+ composite-gate restore)

Motivation (orthogonal-signal division of labour):
  * The diversity term maximizes H_margin -> DELAYS collapse and holds the
    marginal diverse, but on its own only delays (does not prevent) collapse
    on ACDC/Cityscapes (see the divreg lambda sweep).
  * The composite gate keys on DIFFERENT signals (mean_conf trigger +
    grad_norm depth), so it is not redundant with the H_margin the loss
    manipulates (unlike divgate, which would be). It catches what the loss
    only delays, and handles VOC20 (where the diversity term is blind).

CALIBRATION NOTE: adding the diversity term LOWERS mean_conf (predictions
become less peaked), so `conf_ceil` must be RE-CALIBRATED to the divreg
mean_conf peak (lower than plain DeYO-MLMP). gate_log.csv logs mean_conf;
read its peak and set conf_ceil at/just below it. lambda_div=0 recovers the
plain composite gate.

Below is the composite-gate implementation verbatim with the diversity term
added to the loss line (marked). Original docstring follows.
...
class DeYOMLMPDivRegCompositeContinual:
    def __init__(self, ovss_type, ovss_backbone, lr, classes, steps=1,
                 ...
                 reweight_plpd=True,
                 top_block_exclude=6,
                 # --- diversity regularizer (loss-side) ---
                 lambda_div=0.3,
                 # --- composite gate (optimizer-side) ---
                 conf_ceil=0.80,
                 base_rst=0.005,
                 grad_mult_max=4.0,
                 monitor_interval=50,
        ...
        print(f"+++ DeYO-MLMP-DivReg+CompositeGate (T={len(self.prompt_templates)}): "
            ...
            if do_step:
                ...
                if final_mask.sum() > 0:
                    ...
                    if self.reweight_ent or self.reweight_plpd:
                        ...
                        loss = ent_kept.mean()
                    # --- diversity regularizer: maximize H_margin (prob is the
                    # already-differentiable softmax of the adapt logits) ---
