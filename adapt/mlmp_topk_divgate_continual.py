"""
MLMP-TopK-DivGate-Continual: MLMP top-K pixel entropy + 3-tier discrete DivGate.

Same gate machinery as tent_divgate_continual / mlmp_smooth_anchor_continual.
Base loss = mlmp_topk_continual (top-K% confidence pixel mask, optional ILE).
H_margin signal from ensemble probability (mean over T templates).
"""
...
class MLMPTopKDivGateContinual:
    ...
    def __init__(self, ovss_type, ovss_backbone, lr, classes,
        ...
        if self.h_warning > self.h_threshold:
            raise ValueError(f"h_warning > h_threshold")
        ...
        print(f"+++ MLMP-TopK-DivGate: top_k={self.top_k_percent}, "
    ...
    def perform_adaptation(self, x):
        ...
        for _ in range(self.steps):
            ...
            with torch.no_grad():
                ...
                mask = (max_p >= threshold).to(entropy_per_pixel.dtype)
                denom = mask.sum().clamp_min(1.0) * entropy_per_pixel.shape[0]
    ...
    def _update_mode(self):
        ...
        if new_mode != self.current_mode:
            print(f"[MLMP-TopK-DivGate] B{self.total_batches}: H={h:.3f}  "
