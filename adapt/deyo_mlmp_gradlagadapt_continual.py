"""
DeYOMLMPGradLagAdaptContinual — Method 1: GradSlope with the lag cap made
ADAPTIVE to the grad_norm-minimum (best-state) round.

GradSlope sw10+maxlag300 failed on fast collapse because the lag was hard-capped
at 300 iterations: the steep collapse slope wanted lag >> 300 but got clamped, so
the restore was too shallow. GradAnchor (method 2) fixed that by always restoring
toward the fixed best-state anchor with strong rst — but that suppressed the climb
(the strong pull keeps mIoU flat, it never rises to the true peak).

Method 1 keeps GradSlope's *lagged-snapshot* restore (graduated depth), but caps
the lag at the distance back to the grad_norm minimum (the best state):

    lag <= windows_since_min   (how many windows ago the best state was)

Key effect — the cap auto-schedules with the phase:
  * Climb: grad falling -> best state IS ~now -> windows_since_min ~ 0
           -> lag cap ~ 0 -> essentially NO restore -> the climb is free.
  * Post-peak: best state is the peak (several windows back) -> cap grows as we
           drift -> lag can go as deep as the peak. slope sets depth within
           [1, cap], so steep collapse -> deep restore (toward the peak), but
           never PAST the peak.

Snapshots are stored per monitor window (not per batch) to bound memory; lag is
measured in windows.

gate_log.csv columns: total_batches, grad_norm, grad_slope, windows_since_min, lag, rst.
...
class DeYOMLMPGradLagAdaptContinual:
    def __init__(self, ovss_type, ovss_backbone, lr, classes, steps=1,
                 ...
                 base_rst=0.01,
                 max_windows=2000,
                 # two-regime cap: gentle slope -> shallow cap (preserves slow
                 # climb, like sw10+maxlag300); steep slope (>= slope_unlock) ->
                 # adaptive deep cap = grad-min distance (holds fast collapse).
                 # slope_unlock=0 -> always deep (== original method 1).
                 slope_unlock=0.0,
                 maxlag_shallow=6,
                 monitor_interval=50,
                 save_dir=None,
        ...
        self.slope_unlock = float(slope_unlock)
        ...
        print(f"+++ DeYO-MLMP-GradLagAdapt (T={len(self.prompt_templates)}): "
        ...
        if save_dir:
            ...
            with open(self.gate_log_path, 'w') as _f:
                _f.write("total_batches,grad_norm,grad_slope,windows_since_min,lag,rst\n")
    ...
    def _update_gate(self):
        ...
        if slope <= self.slope_deadzone or self.windows_since_min < 1:
            ...
            # two-regime cap: gentle slope keeps a shallow cap (preserve slow
            # climb); steep slope (>= slope_unlock) unlocks the adaptive deep
            # cap = distance back to the best (grad-min) state.
            if slope < self.slope_unlock:
                cap = min(self.maxlag_shallow, self.windows_since_min)
            else:
                cap = self.windows_since_min
            lag = max(1, min(int(round(self.lag_gain * slope)), cap))
            rst = self.base_rst

        print(f"[DeYO-MLMP-GradLagAdapt] B{self.total_batches}: grad={g:.3f} "
              f"slope={slope:+.4f} since_min={self.windows_since_min} "
              f"lag={lag}w rst={rst:.4f}")
        if self.gate_log_path is not None:
            with open(self.gate_log_path, 'a') as _f:
                _f.write(f"{self.total_batches},{g:.6f},{slope:.6f},"
                         f"{self.windows_since_min},{lag},{rst:.6f}\n")
