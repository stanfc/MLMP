"""
MLMP-TopK-SmoothAnchor-Continual: MLMP top-K pixel entropy + continuous
H_margin->lag smooth-anchor gate.

Base loss = mlmp_topk_continual (top-K% confidence pixel mask, optional ILE).
Gate machinery = mlmp_smooth_anchor_continual.
"""
...
class MLMPTopKSmoothAnchorContinual:
    ...
    def __init__(self, ovss_type, ovss_backbone, lr, classes,
        ...
        print(f"+++ MLMP-TopK-SmoothAnchor: top_k={self.top_k_percent}, "
    ...
    def perform_adaptation(self, x):
        ...
        for _ in range(self.steps):
            ...
            with torch.no_grad():
                probs_ens = logits.softmax(dim=2).mean(dim=0)
                self.marginal_buf.append(
                    probs_ens.mean(dim=[0, 2, 3]).detach().float().cpu())
                max_p = probs_ens.max(dim=1).values
    ...
    def _update_lag(self):
        ...
        if log_change:
            print(f"[MLMP-TopK-SA] B{self.total_batches}: H={h:.3f}  "
