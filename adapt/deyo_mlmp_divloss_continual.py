...
class DeYOMLMPDivLossContinual:
    ...
    def perform_adaptation(self, x):
        ...
        for _ in range(self.steps):
            ...
            # --- marginal-diversity regularizer (maximize H_margin) ---
            # Forward, differentiable, single backward: penalize a collapsing
            # class marginal. probs = 7-prompt ensemble softmax; marginal = its
            # mean over batch+spatial; H = entropy of that marginal. We SUBTRACT
            # lambda*H so minimizing the loss pushes the marginal back toward
            # diverse (anti-collapse). Computed over ALL pixels (not the filtered
            # set) so it sees the full prediction's class histogram.
            if self.lambda_div > 0.0:
                probs_ens = logits.softmax(dim=-3).mean(dim=0)        # (B,C,h,w)
                marginal = probs_ens.mean(dim=[0, 2, 3])             # (C,)
                marginal = marginal / marginal.sum().clamp(min=1e-8)
                h_margin = -(marginal * marginal.clamp(min=1e-12).log()).sum()
