...
INPUT_CSV = os.path.join(RESULTS_DIR, "results_v20_mlmp_variants.csv")
OUTPUT_TEX = os.path.join(RESULTS_DIR, "results_v20_mlmp_variants_table.tex")
...
METHOD_DISPLAY = {
    "mlmp_batch1_steps_10_trails_1": "MLMP (batch=1, 10 steps)",
    "mlmp_2": "MLMP (batch=2)",
    "mlmp_continual_64": "MLMP-C (batch=64)",
}

METHOD_ORDER = ["mlmp_batch1_steps_10_trails_1", "mlmp_2", "mlmp_continual_64"]
...
def build_metric_table(data, metric_type="mIoU"):
    ...
    lines.append(f"\\caption{{V20 MLMP Variants Comparison - {metric_display[metric_type]}.}}")
    lines.append(f"\\label{{tab:v20_mlmp_variants_{metric_type.lower()}}}")
