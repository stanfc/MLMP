...
INPUT_CSV = os.path.join(RESULTS_DIR, "results_cityscapes_catseg_methods.csv")
OUTPUT_TEX = os.path.join(RESULTS_DIR, "results_cityscapes_catseg_methods_table.tex")
...
METHOD_ORDER = ["No_Adaptation", "mlmp", "cotta", "mlmp_continual"]
...
def build_metric_table(data, metric_type="mIoU"):
    ...
    lines.append(f"\\caption{{Cityscapes CAT-Seg Method Comparison - {metric_display[metric_type]}.}}")
    lines.append(f"\\label{{tab:cityscapes_catseg_{metric_type.lower()}}}")
