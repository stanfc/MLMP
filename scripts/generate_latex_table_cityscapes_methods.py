...
INPUT_CSV = os.path.join(RESULTS_DIR, "results_cityscapes_methods.csv")
OUTPUT_TEX = os.path.join(RESULTS_DIR, "results_cityscapes_methods_table.tex")
...
METHOD_DISPLAY = {
    ...
    "tent_continual": "Tent-C",
    "dpcore": "DPCore",
}

METHOD_ORDER = ["No_Adaptation", "mlmp", "tent", "cotta", "mlmp_continual", "tent_continual", "dpcore"]
...
def build_metric_table(data, metric_type="mIoU"):
    ...
    lines.append(f"\\caption{{Cityscapes Method Comparison - {metric_display[metric_type]}.}}")
    lines.append(f"\\label{{tab:cityscapes_{metric_type.lower()}}}")
