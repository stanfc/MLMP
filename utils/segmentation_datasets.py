...
def prepare_data(dataset, data_dir, init_resize, patch_size, patch_stride, corruption="original", batch_size=128, num_workers=1, shuffle=True, corruption_cache_dir=None, ann_file=None, split="val", subset_size=None, subset_seed=0, corruption_severity=5, acdc_overlay_corruption=None):
    ...
    # Native-shift datasets (ACDC weather, Dark Zurich, Nighttime Driving, and
    # the DZ+ND combined stream) encode the condition in the data PATH — no
    # synthetic CorruptTransform. ACDC variants additionally support an
    # acdc_overlay_corruption (real shift + synthetic noise layered on top).
    _acdc_conditions = {'fog', 'night', 'rain', 'snow'}
    _acdc_datasets = {"ACDCDataset", "ACDCMerged3Dataset",
                      "ACDCMerged6Dataset", "ACDCMerged10Dataset"}
    _native_shift_datasets = _acdc_datasets | {
        "DarkZurichDataset", "NighttimeDrivingDataset", "DZ_ND_Combined"}
    ...
    elif dataset in _native_shift_datasets:
        print("No corruption added to the pipeline (native-shift dataset)")
